(function (root, factory) {
  ...
  function getAgentTelemetry(agents, filters) {
    ...
    const visibleAgents = source.filter((agent) => {
      const kind = agentStatus(agent);
      ...
      return [agent.name, agent.ip, agent.port, address]
        .some((value) => String(value == null ? '' : value).toLocaleLowerCase().includes(query));
    }).slice();

    const numericDescending = (field) => (a, b) => {
      const diff = Number(b[field] || 0) - Number(a[field] || 0);
      return diff || String(a.name || '').localeCompare(String(b.name || ''), 'zh-CN');
    };
    const statusRank = { offline: 0, busy: 1, online: 2 };
    const comparators = {
      name: (a, b) => String(a.name || '').localeCompare(String(b.name || ''), 'zh-CN'),
      status: (a, b) => statusRank[agentStatus(a)] - statusRank[agentStatus(b)] ||
  ...
  function formatAgentHeartbeat(value, now) {
    const date = new Date(value);
    if (!value || Number.isNaN(date.getTime())) return '—';
    const elapsedSeconds = Math.max(0, Math.floor((new Date(now || Date.now()).getTime() - date.getTime()) / 1000));
    let relative;
